...
const makeRequest = (body: unknown, signature?: string) => {
  ...
  return new NextRequest(
    'http://localhost/api/services/email-verification-email',
    {
      method: 'POST',
      headers,
...
describe('POST /api/services/email-verification-email', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('sends verification email for valid payload', async () => {
    ...
    const req = makeRequest({
      ...
      data: {
        to: 'user@example.com',
        customerName: 'Jane',
        verifyUrl: 'http://localhost/auth/verify-email?token=a&identifier=b',
      },
    })
