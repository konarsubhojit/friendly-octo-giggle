...
describe('GlobalError (app/error.tsx)', () => {
  ...
  it('displays the error message', () => {
    render(<GlobalError error={createError('Network failure')} reset={vi.fn()} />);
    expect(screen.getByText('Network failure')).toBeInTheDocument();
  });

  it('displays error digest when present', () => {
    render(<GlobalError error={createError('fail', 'abc123')} reset={vi.fn()} />);
...
describe('OrdersError (app/orders/error.tsx)', () => {
  ...
  it('displays error digest when present', () => {
    render(<OrdersError error={createError('fail', 'ord-456')} reset={vi.fn()} />);
...
describe('AdminError (app/admin/error.tsx)', () => {
  ...
  it('displays the error message', () => {
    render(<AdminError error={createError('Permission denied')} reset={vi.fn()} />);
    expect(screen.getByText('Permission denied')).toBeInTheDocument();
  });

  it('displays error digest when present', () => {
    render(<AdminError error={createError('fail', 'adm-789')} reset={vi.fn()} />);
...
describe('CartError (app/cart/error.tsx)', () => {
  ...
  it('displays the error message', () => {
    render(<CartError error={createError('Cart service down')} reset={vi.fn()} />);
    expect(screen.getByText('Cart service down')).toBeInTheDocument();
  });

  it('displays error digest when present', () => {
    render(<CartError error={createError('fail', 'cart-012')} reset={vi.fn()} />);
...
describe('ProductsError (app/products/error.tsx)', () => {
  ...
  it('displays the error message', () => {
    render(<ProductsError error={createError('Fetch failed')} reset={vi.fn()} />);
    expect(screen.getByText('Fetch failed')).toBeInTheDocument();
  });

  it('displays error digest when present', () => {
    render(<ProductsError error={createError('fail', 'prod-345')} reset={vi.fn()} />);
