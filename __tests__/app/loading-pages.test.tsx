...
vi.mock('@/components/skeletons/ProductCardSkeleton', () => ({
  default: () => <div data-testid="product-card-skeleton">ProductCardSkeleton</div>,
}));
...
describe('app/products/[id]/loading.tsx – Product Detail Loading', () => {
  ...
  it('renders a product image area skeleton', async () => {
    const { default: ProductDetailLoading } = await import('@/app/products/[id]/loading');
    const { container } = render(<ProductDetailLoading />);
    // Image skeleton has a large rounded container with shadow-2xl
    const imageArea = container.querySelector('.shadow-2xl');
