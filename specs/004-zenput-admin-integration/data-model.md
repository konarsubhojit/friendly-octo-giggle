...
| Price + currency  | `MoneyInput`                  | `currencies={moneyInputCurrencies}` `currency={priceCurrency}` `onCurrencyChange={code => handlePriceCurrencyChange(code as CurrencyCode)}` `value={formData.price}` `onChange={(v) => { setFormData({...formData, price: v ?? 0}); clearFieldError('price') }}` `min={0.01}` `step={0.01}` `label="Price"` `required` `fullWidth` `validationState` `errorMessage` | `formData.price`, `fieldErrors.price`, `priceCurrency`, `availableCurrencies` |
...
| Primary image     | `FileInput`                   | `accept="image/jpeg,image/jpg,image/png,image/webp,image/gif"` `onChange={handleImageChange}` `label="Primary Image (required)"` (conditional in edit mode) `showFileNames` `dropzone` `fullWidth` `previewSrc={imagePreviewUrl ?? formData.image}` `validationState` `errorMessage`                                                                                | `imageFile`, `fieldErrors.image`                                              |
...
| Price + currency              | `MoneyInput`                  | `currencies={moneyInputCurrencies}` `currency={priceCurrency}` `onCurrencyChange={code => handlePriceCurrencyChange(code as CurrencyCode)}` `value={Number(formData.price) \|\| 0}` `onChange={(v) => setFormData(prev => ({...prev, price: String(v ?? 0)}))}` — NOTE: `FormData.price` is `string`; `MoneyInput.value` expects `number`, so the adapter uses `Number()` for display and `String()` on write. `min={0.01}` `step={0.01}` | `formData.price`, `errors.price`, `priceCurrency`   |
| Stock                         | `NumberInput`                 | `value={Number(formData.stock) \|\| 0}` `onChange={(v) => setFormData(prev => ({...prev, stock: String(v ?? 0)}))}` — Same string↔number adapter as Price. `min={0}` `step={1}`                                                                                                                                                                                                                                                           | `formData.stock`, `errors.stock`                    |
| Primary image                 | `FileInput`                   | Same as product image pattern, with `previewSrc={currentPrimaryImagePreview}` and blob URL cleanup via `useEffect`                                                                                                                                                                                                                                                                                                                        | `primaryImageFile`, `errors.image`                  |
