...
| Price (numeric only)          | `NumberInput`                 | `value={Number(formData.price) \|\| 0}` `onChange={(v) => setFormData(prev => ({...prev, price: String(v ?? 0)}))}` — NOTE: `FormData.price` is `string`; `NumberInput.value` expects `number`, so the adapter uses `Number()` for display and `String()` on write. `min={0}` `step={0.01}`                              | `formData.price`, `errors.price`                    |
