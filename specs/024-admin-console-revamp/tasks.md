...
- [x] T099 [P] Consume the remaining resource definitions (`reviews`, `returns`, `checkout-requests`, `email-failures`) from their respective admin screens, one screen per commit, verifying no behavioral regression per screen (depends on T096) — all four screens already wire `AdminDataView`'s `definition` prop from their `createXDefinition` factories (columns/bulk actions consumed directly; filters and row actions mirrored inline as in the orders reference pattern). `returns` keeps its card-based triage queue (via `AdminReturnCard` in `expandedRowRender`/`renderMobileCard`) rather than a plain table, since a per-return action-button card has no table/list conceptual equivalent in `ResourceListDefinition`'s row/column model — this is a deliberate, documented exception, not a skipped conversion.
