...
> **Conditional on [research.md](./research.md) R1.** If the policy conflict is resolved as
> Option B (damaged-item returns only), this set collapses to
> `['DAMAGED', 'DEFECTIVE', 'WRONG_ITEM']` and evidence becomes mandatory.
...
export const CreateReturnRequestSchema = z.object({
  ...
  items: z
    ...
    .min(1)
    .max(50),
  evidenceIds: z.array(z.string().length(7)).max(5).optional(),
})
...
7. Every `evidenceId` must be an orphaned row owned by the caller and scoped to the same order;
   non-matching ids are ignored rather than rejected.
...
`npm run db:migrate`, then `scripts/sql/bootstrap-drizzle-initial.sql` is refreshed per
constitution workflow step 6.
