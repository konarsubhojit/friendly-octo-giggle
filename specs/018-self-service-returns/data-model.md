...
**Images only.** This table never holds video. The policy-mandated video is sent over Instagram
direct message and correlated by return ID ([research.md](./research.md) R15), so no row, column,
or media-type discriminator is needed for it.
...
### M6 — no social-handle column (deliberate omission)

`ReturnRequest` does **not** carry an `instagramHandle` or any other social identifier, and no
migration adds one. FR-020 makes this binding.

**Why**: storing it would introduce a new category of personal data requiring a privacy-policy
change, and the value is unverifiable — nothing prevents a customer entering another person's
handle. Correlation by return ID is sufficient and already available: the ID is unique, already
shown to the customer, and already the natural search key for an administrator scanning the
inbox.

Recorded here as an explicit decision rather than an oversight, so a later reviewer does not
"fix" it by adding the column.
...
`npm run db:migrate`, then schema and migration committed together per constitution workflow
step 6.
