...
- **Continuous integration**: `.github/workflows/build.yml`, whose `test`, `provider-matrix`, `build`, `sonarqube`, `deepsource`, and `codecov` jobs run on every pull request and on pushes to `develop` and `master`. Every job runs on GitHub-hosted runners; this repository uses no self-hosted runner pool. (`provider-matrix` is named for the *provider profile* it exercises — Postgres, node-redis, and MinIO as `services:` containers — not for the runner it runs on.)
