...
- **Continuous integration**: `.github/workflows/build.yml`. Its `test`, `provider-matrix`, `build`, `deepsource`, and `codecov` jobs run on every pull request and on pushes to `develop` and `master`; `sonarqube` is narrower, running only when the branch is `develop` or the pull request targets `develop`. Every job runs on GitHub-hosted runners; this repository uses no self-hosted runner pool. (`provider-matrix` is named for the _provider profile_ it exercises — Postgres and node-redis as `services:` containers, plus MinIO started as a manually managed container because `services:` cannot supply the `server /data` command its image needs — not for the runner it runs on.)
- **SonarQube analysis is owned by `build.yml` only**, and there is deliberately no second workflow duplicating it. A duplicate scan of the same commit would only race the first, and the action is fragile enough on a constrained network that the second copy is a liability: `SonarSource/sonarqube-scan-action` verifies the SonarScanner CLI download against SonarSource's GPG key, which it fetches at run time from `hkps://keyserver.ubuntu.com` (falling back to `hkps://keys.openpgp.org`) into a throwaway keyring. The action takes no input for supplying that key from disk, so any runner that cannot reach those hosts on port 443 fails the job outright with `gpg: keyserver receive failed: Server indicated a failure`. If the scan is ever moved or copied to a network without that egress, open it to those two keyservers or set `HTTPS_PROXY` on the runner — `skipSignatureVerification: true` would also make the job pass, but only by discarding the one check proving the downloaded scanner is the binary SonarSource signed.
