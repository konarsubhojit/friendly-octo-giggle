/**
 * Email service with Google SMTP + SendGrid fallback.
 * Prefers Google SMTP when configured, otherwise uses SendGrid.
 */
...
const sendEmail = async (msg: {
  ...
  if (!hasGoogleSmtp && !hasSendGrid) {
    logBusinessEvent({
      event: "email_skipped",
      details: { to: msg.to, subject: msg.subject, reason: "no_provider_config" },
      success: true,
    });
    return;
  }
  ...
  if (hasGoogleSmtp && smtpTransport) {
    try {
      ...
      // Continue to SendGrid fallback if configured.
    }
  }
