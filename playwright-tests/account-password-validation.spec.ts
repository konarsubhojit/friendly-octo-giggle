...
const goToAccountWithPassword = async (
...
const openChangePasswordForm = async (
  page: import('@playwright/test').Page
) => {
...
test.describe('Account page - read-only by default', () => {
  test('profile information is shown in read-only mode on load', async ({
    page,
  }) => {
    await goToAccountWithPassword(page)
  ...
  test('Cancel button hides the profile edit form', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath('account-edit-form-cancelled'),
    })
  })
  ...
  test('password form is hidden by default', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath('account-password-hidden-by-default'),
    })
  })
  ...
  test('Change Password button reveals the password form', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath('account-password-form-open'),
    })
  })
})

test.describe('Account page - profile form validation', () => {
  test('shows name required error when name is empty on save', async ({
    page,
  }) => {
    await goToAccountWithPassword(page)
    await page.getByRole('button', { name: /edit profile/i }).click()
    ...
    await page.screenshot({
      path: screenshotPath('account-profile-name-error'),
    })
  })
  ...
  test('shows email error for invalid email on save', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath('account-profile-email-error'),
    })
  })
  ...
  test('shows phone error for invalid phone on save', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath('account-profile-phone-error'),
    })
  })
...
test.describe('Account page - confirm new password validation', () => {
  test('no error shown while typing in confirm password field before blur', async ({
    ...
    await page.screenshot({
      path: screenshotPath('account-no-error-while-typing'),
    })
  })

  test('error shown after blurring confirm new password when passwords do not match', async ({
    page,
  }) => {
    await openChangePasswordForm(page)
  ...
  test('error disappears when confirm new password is corrected to match', async ({
    page,
  }) => {
    await openChangePasswordForm(page)
    ...
    await page.screenshot({
      path: screenshotPath('account-error-gone-when-match'),
    })
  })

  test('no error when confirm new password is empty after blur', async ({
    page,
  }) => {
    await openChangePasswordForm(page)
  ...
  test('shows validation errors when submitting empty password form', async ({
    page,
  }) => {
    await openChangePasswordForm(page)
    ...
    await page.screenshot({
      path: screenshotPath('account-password-validation-errors'),
    })
  })
  ...
  test('Cancel button hides the password form', async ({ page }) => {
    ...
    await expect(page.locator('#current-password')).not.toBeVisible()
    await page.screenshot({
      path: screenshotPath('account-password-form-cancelled'),
    })
  })
})
