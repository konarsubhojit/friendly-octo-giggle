...
test('GET /api/orders?limit=3 returns 200 and valid JSON', async ({ request }) => {
...
test('/orders page loads without error', async ({ page }) => {
  ...
  const errorText = page.getByText(/failed to fetch orders|something went wrong|500/i);
  await expect(errorText).not.toBeVisible();
  console.log('✓ No error message shown');
});

// ─── Test 3: Full flow — shop → add to cart → checkout → verify order appears ─
test('full order flow: shop → cart → checkout → orders list', async ({ page }) => {
  await suppressExchangeRates(page);
  ...
  if (await addToCartBtn.isVisible({ timeout: 5000 })) {
    ...
    console.log('Add to cart button not found — product may not exist in DB yet');
...
test('account page recent orders section loads without error', async ({ page }) => {
  await suppressExchangeRates(page);
