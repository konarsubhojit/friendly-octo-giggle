...
test.describe("Mobile Navigation - Hamburger Menu", () => {
  test("hamburger button is visible on mobile viewport", async ({
    ...
    await page.screenshot({
      path: screenshotPath("mobile-home-closed-nav"),
      fullPage: false,
    });
  });

  test("hamburger button is hidden on desktop viewport", async ({
    page,
  }, testInfo) => {
    test.skip(
    ...
    await page.screenshot({
      path: screenshotPath("desktop-home-no-hamburger"),
      fullPage: false,
    });
  });

  test("mobile nav drawer opens and shows links on click", async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== "mobile-chrome",
    ...
    await expect(
      page.getByRole("link", { name: /^contact$/i }).last(),
    ).toBeVisible();

    await page.screenshot({
      path: screenshotPath("mobile-home-open-nav"),
      fullPage: false,
    });
  });

  test("mobile nav drawer closes when hamburger clicked again", async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== "mobile-chrome",
    ...
    await page.screenshot({
      path: screenshotPath("mobile-home-closed-nav-again"),
      fullPage: false,
    });
  });

  test("hamburger button has correct aria attributes", async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== "mobile-chrome",
...
test.describe("ProductGrid – search and category filter", () => {
  ...
  test("search input accepts text and filters results", async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== "desktop-chrome",
