...
const mockCartApi = async (page: Page) => {
  await page.route('**/api/cart**', (route) => {
    ...
    if (method === 'GET') {
      ...
      route.fulfill({
        json: { success: true, data: { cart: { id: 'c1', items: [] } } },
      })
    } else {
      route.continue()
    }
...
test.describe('Homepage – Bestsellers ProductGrid (no DB)', () => {
  ...
  test('"All" pill is selected by default (aria-pressed=true)', async ({
    page,
  }) => {
    await mockExchangeRates(page)
    await page.goto('/')
  ...
  test('clicking a category pill updates aria-pressed', async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath('home-category-flowers-selected'),
    })
  })
...
test.describe('Admin Products page – product cards with mock data', () => {
  ...
  test('product cards show product names and stock', async ({ page }) => {
    ...
    await expect(page.getByText('Rose Keyring Set').first()).toBeVisible({
      timeout: 10_000,
    })
...
test.describe('QuickAddButton – rendered in admin products with mock data', () => {
  test('add-to-cart button is visible on an in-stock product card', async ({
    page,
  }) => {
    await mockExchangeRates(page)
    await mockAdminProductsApi(page)
    ...
    const addBtn = page
      .getByRole('button', {
        name: new RegExp(`Add ${firstProduct.name} to cart`, 'i'),
      })
      .first()
