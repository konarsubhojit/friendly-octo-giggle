...
test.describe('Fixed background image — warm-bg.jpeg', () => {
  ...
  test('bg-warm-gradient utility includes fixed warm-bg image and color-mix overlay', () => {
    ...
    expect(css).toContain(
      'color-mix(in srgb, var(--accent-blush) 60%, transparent)'
    )
    expect(css).toContain(
      'color-mix(in srgb, var(--accent-cream) 60%, transparent)'
    )
  })
  ...
  test('fixed background is consistent across multiple pages', async ({
    page,
  }) => {
    ...
    for (const url of pages) {
      ...
      expect(bgAttachment, `Expected fixed attachment on ${url}`).toContain(
        'fixed'
      )
    }
  })
  ...
  test('background stays fixed after scrolling — about page (desktop)', async ({
    page,
  }, testInfo) => {
    test.skip(
    ...
    await page.screenshot({
      path: screenshotPath('fixed-bg-about-top'),
      fullPage: false,
    })
    ...
    await page.screenshot({
      path: screenshotPath('fixed-bg-about-scrolled'),
      fullPage: false,
    })
  ...
  test('background stays fixed after scrolling — blog page (mobile)', async ({
    page,
  }, testInfo) => {
    test.skip(
    ...
    await page.screenshot({
      path: screenshotPath('fixed-bg-mobile-blog-top'),
      fullPage: false,
    })
    ...
    await page.screenshot({
      path: screenshotPath('fixed-bg-mobile-blog-scrolled'),
      fullPage: false,
    })
  ...
  test('full page screenshot — blog with fixed background (desktop)', async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== 'desktop-chrome',
  ...
  test('full page screenshot — about with fixed background (desktop)', async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== 'desktop-chrome',
  ...
  test('full page screenshot — contact with fixed background (mobile)', async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== 'mobile-chrome',
  ...
  test('full page screenshot — shipping with fixed background (mobile)', async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== 'mobile-chrome',
  ...
  test('bg-warm-gradient overlay is semi-transparent (background has gradient + image)', async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== 'desktop-chrome',
