 ...
 * - "The two passwords need to match exactly." message does NOT appear while typing (before blur)
 * - "The two passwords need to match exactly." message appears after the field is blurred when passwords differ
...
const registerMismatchMessage = "The two passwords need to match exactly.";
...
test.describe("Register page - password validation", () => {
  test("no error shown while typing in confirm password field before blur", async ({
    ...
    await expect(page.getByText(registerMismatchMessage)).not.toBeVisible();

    await page.screenshot({
      path: screenshotPath("register-no-error-while-typing"),
    });
  });
  ...
  test("error shown after blurring confirm password when passwords do not match", async ({
    ...
    await expect(page.getByText(registerMismatchMessage)).toBeVisible();

    await page.screenshot({
      path: screenshotPath("register-error-after-blur"),
    });
  });
  ...
  test("error disappears when passwords match after blur", async ({ page }) => {
    ...
    await page.screenshot({
      path: screenshotPath("register-error-gone-when-match"),
    });
  });
  ...
  test("only one error message shown for confirm password at a time", async ({
    ...
    // There should be exactly one mismatch message
    const errorMessages = page.getByText(registerMismatchMessage);
    await expect(errorMessages).toHaveCount(1);

    await page.screenshot({
      path: screenshotPath("register-single-error-message"),
    });
  });
  ...
  test("stale server-side error clears when user edits confirm password field", async ({
    ...
    await page.route("**/api/auth/register", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          ...
          details: { [confirmPasswordKey]: registerMismatchMessage },
        }),
      }),
    );
    ...
    await page.screenshot({
      path: screenshotPath("register-stale-error-cleared"),
    });
  });
});
