    ...
    <tr><td align="center" style="padding:40px 20px;">
      <table role="presentation" width="600" style="max-width:600px;background:#FFFDFB;border-radius:16px;box-shadow:0 4px 20px rgba(92,74,68,0.08);overflow:hidden;">
        ...
        <tr><td style="background:linear-gradient(135deg,#b83060,#cc4880);padding:32px 40px;text-align:center;">
          ...
          <p style="color:rgba(255,255,255,0.85);margin:8px 0 0;font-size:14px;">${labels.handmadeWithLove}</p>
        </td></tr>
        <!-- Body -->
        ...
        <tr><td style="padding:24px 40px;text-align:center;border-top:1px solid #F2E8E4;">
          <p style="margin:0;color:#7a5543;font-size:13px;">© ${new Date().getFullYear()} ${STORE_NAME}. ${labels.allRightsReserved}</p>
          <p style="margin:8px 0 0;color:#7a5543;font-size:12px;">${labels.handcrafted}</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
<table role="presentation" width="100%" style="border-collapse:collapse;margin:16px 0;">
  <thead>
    <tr style="background:#F9F0EB;">
      <th style="text-align:left;padding:10px 12px;color:#5C4A44;font-size:13px;font-weight:600;border-bottom:2px solid #E8D5CC;">${labels.item}</th>
      <th style="text-align:center;padding:10px 12px;color:#5C4A44;font-size:13px;font-weight:600;border-bottom:2px solid #E8D5CC;">${labels.qty}</th>
      <th style="text-align:right;padding:10px 12px;color:#5C4A44;font-size:13px;font-weight:600;border-bottom:2px solid #E8D5CC;">${labels.price}</th>
    </tr>
  </thead>
  <tbody>
    ${items
...
export const orderStatusUpdateTemplate = (data: OrderStatusUpdateData) => {
  ...
  const statusText = labels[info.label as keyof typeof labels] ?? info.label
