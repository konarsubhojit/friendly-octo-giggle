 ...
 *
 * Shared by the nightly cron and admin "recompute now" registrations so the
 * two entry points execute exactly the same code.
 *
 ...
 */
const createComputeProductAffinityFunction = (includeCronTrigger: boolean) =>
  inngest.createFunction(
    {
      id: 'compute-product-affinity',
      name: 'Compute product affinity scores',
      triggers: includeCronTrigger
        ? [cron('0 4 * * *'), { event: AFFINITY_RECOMPUTE_EVENT }]
        : [{ event: AFFINITY_RECOMPUTE_EVENT }],
      concurrency: { limit: 1 },
    ...
    async ({ event, step }) => {
      ...
      const triggeredBy =
        typeof eventData.triggeredBy === 'string'
...
export const computeProductAffinityFunction =
  createComputeProductAffinityFunction(true)

export const computeProductAffinityEventFunction =
  createComputeProductAffinityFunction(false)
