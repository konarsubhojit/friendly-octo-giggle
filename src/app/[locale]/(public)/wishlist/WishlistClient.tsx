...
interface WishlistClientProps {
  ...
  readonly initialProductIds: string[]
}
...
const WishlistClient = ({
  ...
  const hydrated = useSelector((state: RootState) => state.wishlist.hydrated)
  ...
  // Seed the store with the server-fetched wishlist once. Until then, render
  // directly from the server props so both SSR and the first client render
  // show the same content (no empty-state flash, no hydration mismatch).
  useEffect(() => {
    dispatch(
      hydrateWishlist({
  ...
  }, [dispatch, initialProducts, initialProductIds])

  const products = hydrated ? reduxProducts : initialProducts
