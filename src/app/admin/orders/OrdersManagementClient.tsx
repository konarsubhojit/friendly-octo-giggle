...
import {
  createOrdersDefinition,
  type OrderRow as OrderDefinitionRow,
} from '@/features/admin/resources/orders'
import type {
  BulkResult,
  BulkSelection,
...
type OrderRow = OrderDefinitionRow
...
export default function OrdersManagementClient({
  ...
  const resolveBulkOrderIds = useCallback(
    async (selection: BulkSelection): Promise<string[]> => {
      ...
      if (snapshot.status && snapshot.status !== 'ALL') {
      ...
      const data = await res.json().catch(() => ({}))
      const items: AdminOrder[] = data.data?.orders ?? data.orders ?? []
      return items.map((order) => order.id)
    },
    [totalCount]
  )
  ...
  const applyBulkStatus = useCallback(
    async (
      ...
      const orderIds = await resolveBulkOrderIds(selection)
      ...
      try {
        ...
        const data = await res.json().catch(() => ({}))
        if (!res.ok) {
          const reason = data.error ?? 'Failed to update orders'
          return {
            succeeded: [],
            failed: orderIds.map((rowId) => ({ rowId, reason })),
          }
        }
        setOrders((prev) =>
          prev.map((o) =>
  ...
  const ordersDefinition = useMemo(
    () =>
      createOrdersDefinition(permissions, {
        onMarkShipped: (selection) =>
          applyBulkStatus(selection, OrderStatus.SHIPPED),
        onBulkCancel: (selection) =>
          applyBulkStatus(selection, OrderStatus.CANCELLED),
        // Row actions aren't rendered by AdminDataView yet (only columns and
        // bulk actions are consumed today) — the equivalent affordances are
        // handled inline via AdminOrderCard in the expanded row below. See
        // specs/024-admin-console-revamp/tasks.md for tracked follow-up.
        onRefund: () => {},
        onViewDetail: () => {},
        onUpdateStatus: () => {},
      }),
    [permissions, applyBulkStatus]
  )
  ...
  const renderExpandedOrder = (row: OrderRow) => {
    const order = orders.find((candidate) => candidate.id === row.id)
  ...
  const ordersListContent = (
    <AdminDataView
      ...
      filterSnapshot={{ search, status: filter }}
